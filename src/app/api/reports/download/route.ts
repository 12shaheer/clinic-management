...
import type { DateRange } from "@/app/(dashboard)/reports/actions";

const VALID_RANGES = ["today", "yesterday", "last_7_days", "last_30_days", "last_6_months", "last_year", "all_time"];

function getDateFilter(range: DateRange): string | null {
  const now = new Date();
  switch (range) {
    case "today": return now.toISOString().split("T")[0];
    case "yesterday": { const d = new Date(now); d.setDate(d.getDate() - 1); return d.toISOString().split("T")[0]; }
    case "last_7_days": { const d = new Date(now); d.setDate(d.getDate() - 7); return d.toISOString().split("T")[0]; }
    case "last_30_days": { const d = new Date(now); d.setDate(d.getDate() - 30); return d.toISOString().split("T")[0]; }
    case "last_6_months": { const d = new Date(now); d.setMonth(d.getMonth() - 6); return d.toISOString().split("T")[0]; }
    case "last_year": { const d = new Date(now); d.setFullYear(d.getFullYear() - 1); return d.toISOString().split("T")[0]; }
...
export async function GET(req: NextRequest) {
  const range = req.nextUrl.searchParams.get("range") as DateRange;
  ...
  const headerStyle = { font: { bold: true, color: { rgb: WHITE }, sz: 11, name: "Calibri" }, fill: { fgColor: { rgb: BLUE } }, alignment: { horizontal: "center" as const, vertical: "center" as const }, border: borders };
  const titleStyle = { font: { bold: true, color: { rgb: BLUE }, sz: 14, name: "Calibri" }, alignment: { horizontal: "left" as const, vertical: "center" as const } };
  const subtitleStyle = { font: { bold: true, color: { rgb: BLUE }, sz: 11, name: "Calibri" }, fill: { fgColor: { rgb: LIGHT_BLUE } }, alignment: { horizontal: "left" as const, vertical: "center" as const }, border: borders };
  const cellStyle = { font: { sz: 10, name: "Calibri" }, alignment: { vertical: "center" as const }, border: borders };
  ...
  row++;
  for (const [label, val] of [["Total Patients", summary.totalPatients], ["Total Appointments", summary.totalAppointments], ["Completed", summary.completedAppointments], ["Cancelled", summary.cancelledAppointments], ["Checked In", summary.checkedIn]] as const) {
    const isAlt = row % 2 === 0;
  ...
  ws[XLSX.utils.encode_cell({ r: row, c: 1 })] = { v: "", t: "s", s: subtitleStyle };
  row++;
  for (const [label, val] of [["Total Invoiced (PKR)", summary.totalInvoiced], ["Revenue Collected (PKR)", summary.totalRevenue], ["Outstanding (PKR)", summary.outstandingAmount], ["Total Invoices", summary.totalInvoices], ["Unpaid Invoices", summary.unpaidInvoices]] as const) {
    const isAlt = row % 2 === 0;
    const bg = isAlt ? { fill: { fgColor: { rgb: GRAY_BG } } } : {};
    ws[XLSX.utils.encode_cell({ r: row, c: 0 })] = { v: label, t: "s", s: { ...metricLabelStyle, ...bg } };
  ...
  for (const inv of (invoices ?? [])) {
    ...
    if (inv.status === "paid") patientPaidMap[inv.patient_id] = (patientPaidMap[inv.patient_id] || 0) + (inv.total || 0);
  }
  const patientRows = (patients ?? []).map((p: any) => {
    const invoiced = patientInvoicedMap[p.id] || 0;
    const paid = patientPaidMap[p.id] || 0;
    return [p.patient_code, `${p.first_name} ${p.last_name}`, p.phone, p.gender || "—", (p.status as string).charAt(0).toUpperCase() + (p.status as string).slice(1), p.credit_balance || 0, invoiced, paid, invoiced - paid, p.created_at?.split("T")[0] || ""];
  });
  XLSX.utils.book_append_sheet(wb, buildStyledSheet(["Code", "Name", "Phone", "Gender", "Status", "Credit Balance", "Total Invoiced", "Total Paid", "Dues", "Registered"], patientRows, [12, 22, 16, 10, 10, 16, 16, 14, 14, 14], [5, 6, 7, 8], [3, 4]), "Patients");
  ...
  const apptRows = (appointments ?? []).map((a: any) => {
    const pat = a.patients as any; const phy = a.physiotherapists as any;
    return [a.appointment_code, pat ? `${pat.first_name} ${pat.last_name}` : "", pat?.phone || "", phy ? `Dr. ${phy.first_name} ${phy.last_name}` : "", a.appointment_date, a.start_time, a.appointment_type || "—", statusLabel(a.status)];
  });
  XLSX.utils.book_append_sheet(wb, buildStyledSheet(["Code", "Patient", "Phone", "Physiotherapist", "Date", "Time", "Type", "Status"], apptRows, [14, 22, 16, 24, 14, 10, 14, 14], [], [4, 5, 7]), "Appointments");

  // Invoices
  const collectedByLabel = (v: string | null) => { if (!v) return ""; if (v === "reception") return "At Reception"; if (v === "doctor") return "By Doctor"; if (v === "credit") return "Credit Settlement"; return v; };
  const invRows = (invoices ?? []).map((i: any) => {
    const pat = i.patients as any;
    return [i.invoice_code, pat ? `${pat.first_name} ${pat.last_name}` : "", pat?.phone || "", i.subtotal, i.discount, i.total, i.status === "paid" ? "Paid" : i.status === "partially_paid" ? "Partially Paid" : i.status === "cancelled" ? "Cancelled" : "Unpaid", i.status === "paid" ? collectedByLabel(i.collected_by) : "", i.payment_confirmed_at?.split("T")[0] || "", i.issued_at?.split("T")[0] || ""];
  });
  XLSX.utils.book_append_sheet(wb, buildStyledSheet(["Code", "Patient", "Phone", "Subtotal", "Discount", "Total", "Status", "Collected By", "Payment Date", "Issued Date"], invRows, [14, 22, 16, 14, 14, 14, 14, 18, 14, 14], [3, 4, 5], [6, 8, 9]), "Invoices");
